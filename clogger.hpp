...
inline Clogger::Clogger(const char *fp) : fp(fp) {
...
// helper function to resize the log file size
...
// worker thread that runs constantly in the background to check new data from this->m_vec to add to log 
